...
async function loadState() { const state = await call("/api/state"); $("mode").textContent = `${state.mode} mode`; $("tenantDid").textContent = state.tenantDid; $("agentDid").textContent = state.agentDid; $("purpose").textContent = state.purpose; $("piiStatus").textContent = state.piiStatus; $("handoffId").textContent = state.handoffId || "no handoff"; }
